...
class Mural(db.Model):
    """Board where tasks are organized by sector/tag."""
...
class MuralTarefa(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    mural_id = db.Column(
        db.Integer, db.ForeignKey("murais.id"), nullable=False, index=True
    )
...
class MuralTarefaHistorico(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    tarefa_id = db.Column(
        db.Integer, db.ForeignKey("mural_tarefas.id"), nullable=False, index=True
    )
