...
def update_meeting(form, raw_events, now, meeting: Reuniao):
    ...
    start_dt = datetime.combine(form.date.data, form.start_time.data).replace(tzinfo=BRASILIA_TZ)
    end_dt = datetime.combine(form.date.data, form.end_time.data).replace(tzinfo=BRASILIA_TZ)
    duration = end_dt - start_dt
    intervals: list[tuple[datetime, datetime]] = []
    for e in raw_events:
    ...
    for r in Reuniao.query.filter(Reuniao.id != meeting.id).all():
        existing_start = datetime.combine(r.data_reuniao, r.hora_inicio).replace(tzinfo=BRASILIA_TZ)
        existing_end = datetime.combine(r.data_reuniao, r.hora_fim).replace(tzinfo=BRASILIA_TZ)
        intervals.append((existing_start, existing_end))
    proposed_start = max(start_dt, now + MIN_GAP)
    messages: list[str] = []
    if proposed_start != start_dt:
...
def combine_events(raw_events, now, current_user_id: int):
    ...
    for e in raw_events:
        ...
        start_dt = isoparse(start_str).astimezone(BRASILIA_TZ)
        end_dt = isoparse(end_str).astimezone(BRASILIA_TZ)
