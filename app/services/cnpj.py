...
def regime_to_tributacao(value) -> str:
    ...
    if isinstance(value, dict):
        for k in ("id", "codigo", "cod", "value", "descricao", "regime"):
            if k in value and value[k] not in (None, "", [], {}):
                res = regime_to_tributacao(value[k])
                if res:
                    return res
        return ""
    if isinstance(value, list):
        for item in value:
            res = regime_to_tributacao(item)
            if res:
                return res
        return ""
    s = str(value).strip()
...
def consultar_cnpj(cnpj_input: str) -> dict | None:
    ...
    if base:
        ...
        trib = regime_to_tributacao(
            deep_pick(
                base,
                {"regime", "regime_tributario", "regimeid", "regime_id", "tributacao"},
            )
        )
        if trib:
