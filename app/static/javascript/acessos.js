function setupAcessos(containerId, addBtnId, hiddenInputId) {
  ...
  function render() {
    ...
    acessos.forEach((p, idx) => {
      ...
      row.innerHTML = `
        <div class="col-md-3"><input type="text" class="form-control acesso-acesso" placeholder="Acesso" value="${p.acesso || ''}"></div>
  ...
  function updateHidden() {
    ...
    acessos = Array.from(items).map(item => ({
      acesso: item.querySelector('.acesso-acesso').value,
