...
from app import db
from app.models.mural import Mural, MuralTarefa, StatusTarefa, Prioridade
from app.controllers.routes import admin_required
from app.utils.permissions import user_pode_atuar_no_mural
...
def index():
    ...
    if current_user.role != "admin":
        meus = [m for m in murais if any(us.id == m.setor_id for us in current_user.setores)]
        if not meus:
    ...
    contagens = []
    for m in murais:
        q = MuralTarefa.query.filter_by(mural_id=m.id)
        contagens.append(
            {
                "mural": m,
...
@login_required
def board(slug):
    mural = Mural.query.filter_by(slug=slug, habilitado=True).first_or_404()
    if not user_pode_atuar_no_mural(mural.id):
        abort(403)

    status_filtro = request.args.get("status")
    query = MuralTarefa.query.filter_by(mural_id=mural.id)
    ...
    return render_template("mural/board.html", mural=mural, tarefas=tarefas, status_filtro=status_filtro)
...
def nova():
    if request.method == "POST":
        mural_id = int(request.form["mural_id"])
        mural = Mural.query.get_or_404(mural_id)
        ...
        return redirect(url_for("mural.board", slug=mural.slug))

    murais = Mural.query.filter_by(habilitado=True).order_by(Mural.nome).all()
    return render_template("mural/nova.html", murais=murais)
...
def toggle_andamento(tarefa_id):
    ...
    if not user_pode_atuar_no_mural(tarefa.mural_id):
        abort(403)
